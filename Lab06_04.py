...
metadata_start = getMetaData(base_name + ".json")
start_time = metadata_start['t_start']
...
for row, file in enumerate(files):
    ...
    mapData[row] = power
    time = (metadata['t_start'] - start_time)/3600.0
    times.append(time)
    vLSR = metadata['vlsr']
...
#Fax.plot(calculatedvLSR, times, color='white', marker='o', linestyle='-', linewidth=2, markersize=5, label='vLSR')
