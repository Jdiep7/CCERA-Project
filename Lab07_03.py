...
for i, file in enumerate(files) :
    ...
    longitude.append(gLon)
    v_uncorrected = ''
    
    for i, p in enumerate(power):
        if p > 5.:
            vPrime.append(vDoppler[i])
            v_uncorrected = vDoppler[i]
            break
    
    v_corrected = vLSR - v_uncorrected
    v_r = v_corrected + R*Omega*np.sin(np.radians(gLon))
    radius.append(R*np.sin(np.radians(gLon)))
    vRotation.append(v_r)

aRotation = [a/b for a,b in zip(vRotation, radius)]
        

print("Rotational Velocity: ", vRotation)
print("Radius: ", radius)
...
ax.plot(vPrime, glonList, color='white', linestyle='-', linewidth=2, markersize=5, label='vPrime')
ax.legend()
plt.show()

plt.figure()
plt.scatter(radius, vRotation, color='blue')
plt.xlabel("Galactic Radius (km)")
plt.ylabel("Rotational Velocity v(r) (km/s)")
plt.title("Galactic Rotation Curve")
plt.grid(True)
plt.show()

plt.figure()
plt.scatter(radius, aRotation, color='green')
plt.xlabel("Galactic Radius (km)")
plt.ylabel("Angular Velocity Ω(r) (s^-1)")
plt.title("Galactic Angular Velocity vs Radius")
plt.grid(True)
plt.show()
